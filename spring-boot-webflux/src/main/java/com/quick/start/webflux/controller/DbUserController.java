...
public class DbUserController {
    ...
    public Flux<UserVO> list() {
        ...
        return userRepository.findAll()
                .map(userDO -> new UserVO().setId(Math.toIntExact(userDO.getId())).setUsername(userDO.getUsername()));
    }
                    ...
                    public Mono<Integer> apply(UserDO userDO) {
                        ...
                        return userRepository.save(userDO).flatMap(new Function<UserDO, Mono<Integer>>() {
                            ...
                            public Mono<Integer> apply(UserDO userDO) {
                                ...
                                // 返回编号
                                return Mono.just(Math.toIntExact(userDO.getId()));
                            }
                        });
                    }
    ...
    public Mono<Boolean> update(UserUpdateDTO updateDTO) {
        // 查询用户
        Mono<UserDO> user = userRepository.findById(Long.valueOf(updateDTO.getId()));
    ...
    public Mono<Boolean> delete(@RequestParam("id") Long id) {
        // 查询用户
